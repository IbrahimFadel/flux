...
impl<'a> MirContext<'a> {
	...
	fn lower_stmt(&mut self, block: &mut Block, stmt: &flux_ast::Stmt) -> Vec<Instruction> {
		match stmt {
	...
	fn lower_var_decl(&mut self, var: &flux_ast::VarDecl) -> Vec<Instruction> {
		// i32 x = 0;
		// %0 = StackAlloc(i32)
		// Assign(0, %0)
	...
	fn type_expr_to_mir_type(ty: &flux_ast::Expr) -> Type {
		match ty {
			flux_ast::Expr::PrimitiveType(x) => match x.kind {
				flux_ast::PrimitiveKind::I64 => Type::I64,
				flux_ast::PrimitiveKind::I32 => Type::I32,
				_ => Type::I32,
			},
			_ => Type::I32,
		}
...
impl<'a> MirBuilder<'a> {
	...
	pub fn new_function(&self, name: String, params: Vec<flux_ast::FnParam>) -> FnDecl {
