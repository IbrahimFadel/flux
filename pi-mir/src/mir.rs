...
impl Block {
	pub fn lower_stmt(&mut self, stmt: &pi_ast::Stmt) {
	...
	fn lower_var_decl(&mut self, var: &pi_ast::VarDecl) {
	...
	fn lower_expr(&self, expr: &pi_ast::Expr) -> RValue {
		/*

			i32, u32, etc. not just intlit
		*/
		match expr {
			pi_ast::Expr::IntLit(x) => match self.expecting_type {
			...
			pi_ast::Expr::BinOp(binop) => self.lower_binop(binop),
			_ => RValue::Null,
		}
	}

	fn lower_binop(&self, binop: &pi_ast::BinOp) -> RValue {
		RValue::BinOp(BinOp {
