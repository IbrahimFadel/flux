...
llvm::Module *ssc::codegenNodes(Nodes nodes, unique_ptr<CodegenContext> codegenContext)
{
    for (auto &node : nodes)
    {
...
llvm::Value *FunctionDeclaration::codegen(const unique_ptr<CodegenContext> &codegenContext)
{
    ...
    codegenContext->print(f);

    return 0;
}

void FunctionDeclaration::createFunctionParamAllocas(const unique_ptr<CodegenContext> &codegenContext, llvm::Function *f)
{
...
llvm::Function *FunctionDeclaration::codegenPrototype(const unique_ptr<CodegenContext> &codegenContext)
{
    ...
    llvm::Function *f = llvm::Function::Create(functionType, linkage, name, codegenContext->getModule().get());
...
llvm::Value *VariableDeclaration::codegen(const unique_ptr<CodegenContext> &codegenContext)
{
    auto builder = codegenContext->getBuilder();
    ...
    codegenContext->setCurrentlyPreferredType(type);
...
llvm::Value *BinaryOperationExpression::codegen(const unique_ptr<CodegenContext> &codegenContext)
{
    switch (op)
    {
...
llvm::Value *BinaryOperationExpression::codegenBinopSumDiffProdQuot(const unique_ptr<CodegenContext> &codegenContext, unique_ptr<Expression> lhs, unique_ptr<Expression> rhs, TokenType op)
{
    auto builder = codegenContext->getBuilder();
    auto lVal = lhs->codegen(codegenContext);
    auto rVal = rhs->codegen(codegenContext);

    auto preferredType = codegenContext->getCurrentlyPreferredType();
    auto llvmPreferredType = codegenContext->ssTypeToLLVMType(preferredType);
    ...
    case TokenType::tokSlash:
    {
        if (codegenContext->isTypeSigned(preferredType))
...
llvm::Value *NumberExpression::codegen(const unique_ptr<CodegenContext> &codegenContext)
{
    auto llvmPreferredType = codegenContext->ssTypeToLLVMType(codegenContext->getCurrentlyPreferredType());
    if (llvmPreferredType->isFloatTy() || llvmPreferredType->isDoubleTy())
    {
...
// unique_ptr<llvm::Module> codegenNodes(Nodes _nodes)
// {
...
// {
//     auto f = codegenPrototype();
//     if (f == 0)
//         error("Could not codegen" + name + "'s function prototype");

//     auto entry = llvm::BasicBlock::Create(ctx, "entry", f);
//     builder->SetInsertPoint(entry);

//     currentFunctionName = name;
//     functions[currentFunctionName] = this;

//     createFunctionParamAllocas(f, parameters);
// }

// llvm::Function *FunctionDeclaration::codegenPrototype()
// {
//     std::vector<llvm::Type *> paramTypes;
//     for (auto &param : parameters)
//     {
//         auto ty = ssTypeToLLVMType(param.type);
//         paramTypes.push_back(ty);
//     }

//     auto llvmReturnType = ssTypeToLLVMType(returnType);

//     llvm::FunctionType *functionType = llvm::FunctionType::get(llvmReturnType, paramTypes, false);

//     auto linkage = pub ? llvm::Function::ExternalLinkage : llvm::Function::PrivateLinkage;
//     llvm::Function *f = llvm::Function::Create(functionType, linkage, name, module.get());
...
// llvm::Value *BinaryOperationExpression::codegen()
// {
//     return 0;
// }
...
//     {
//         auto ptr = builder->CreateAlloca(ssTypeToLLVMType(param.type), 0, param.name);
//         auto store = builder->CreateStore(functionParamIt, ptr);
//         auto loaded = builder->CreateLoad(ptr);
//         if (param.mut)
//         {
//             functions[currentFunctionName]->setMutable(param.name, loaded);
//         }
//         else
//         {
//             functions[currentFunctionName]->setConstant(param.name, loaded);
//         }
...
// // void CodeGenerator::codegenVariableDeclaration(unique_ptr<Node> varDecNode)
// // {
// // }
