...
public:
    ...
    virtual std::map<std::string, std::unique_ptr<Expression_Node>> get_properties() = 0;
};
...
public:
    ...
    std::map<std::string, std::unique_ptr<Expression_Node>> get_properties();
};
...
public:
    ...
    Variable_Node(std::string name, Variable_Types type, std::string object_type, std::unique_ptr<Expression_Node> value) : name(name), type(type), object_type(object_type), value(std::move(value)){};
    llvm::Value *code_gen();
...
class Object_Node : public Expression_Node
{
private:
    std::string name;
...
public:
    Object_Node(std::string name, std::map<std::string, Variable_Types> properties) : name(name), properties(properties){};
    llvm::Value *code_gen();
    std::map<std::string, std::unique_ptr<Expression_Node>> get_properties();
};

class Object_Expression_Node : public Expression_Node
{
private:
    std::map<std::string, std::unique_ptr<Expression_Node>> properties;

public:
    Object_Expression_Node(std::map<std::string, std::unique_ptr<Expression_Node>> properties) : properties(std::move(properties)){};
    llvm::Value *code_gen();
    std::map<std::string, std::unique_ptr<Expression_Node>> get_properties();
};
...
static std::unique_ptr<Expression_Node> parse_object();
static std::unique_ptr<Expression_Node> parse_object_expression();
