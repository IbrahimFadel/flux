...
// 	pub fn check_expr(&mut self, expr: &'a mut Expr) -> Option<FluxError> {
// 		match expr {
...
// 	fn check_struct_expr(&self, struct_expr: &mut StructExpr) -> Option<FluxError> {
...
// 	fn check_call(&self, call: &mut CallExpr) -> Option<FluxError> {
// 		match &**call.callee {
// 			Expr::BinOp(binop) => {
// 				let (expr, err) =
...
// 		&self,
// 		binop: &Spanned<BinOp>,
// 	) -> (Option<Box<Spanned<Expr>>>, Option<FluxError>) {
// 		match &**binop.x {
...
// 	fn check_binop_double_colon_call(&self, binop: &BinOp) -> Option<FluxError> {
...
// 	fn check_binop(&mut self, binop: &'a mut BinOp) -> Option<FluxError> {
// 		match binop.op {
...
// 	fn check_binop_double_colon(&self, binop: &'a BinOp) -> Option<FluxError> {
// 		None
// 	}

// 	fn check_binop_eq(&mut self, binop: &'a BinOp) -> Option<FluxError> {
// 		match &**binop.x {
// 			Expr::BinOp(b) => {
...
// 	fn get_struct_access_type(&mut self, binop: &'a BinOp) -> (&'a Expr, Option<FluxError>) {
// 		let mut b = binop;
...
// 	fn check_float_lit(&mut self, float: &mut FloatLit) -> Option<FluxError> {
...
// 	fn check_int_lit(&mut self, int: &mut IntLit) -> Option<FluxError> {
// 		if let Some(Expr::PrimitiveType(prim)) = &self.expecting_ty {
...
// 	fn reassign_int_lit_bits(&self, int: &mut IntLit, prim: &PrimitiveType) -> Option<FluxError> {
// 		let expected_bits = primitive_kind_to_bits(&prim.kind);
