...
pub struct TraitApplicationTable {
    // table: HashMap<u32, Vec<(TraitApplication, InFile<Span>)>>,
    table: HashMap<u32, Vec<TraitApplication>>,
}
...
impl TraitApplicationTable {
    ...
    pub fn push_application(&mut self, trait_id: u32, application: TraitApplication) {
        self.table.entry(trait_id).or_default().push(application);
    }

    pub fn get_applications(&self, trait_id: u32) -> Option<&Vec<TraitApplication>> {
        self.table.get(&trait_id)
    }

    pub fn set_application_span(
        &mut self,
        trait_id: u32,
        implementation: TraitApplication,
        span: InFile<Span>,
    ) {
        // self.spans
        //     .entry(trait_id)
    ...
    // pub fn get_application_span(&self, trait_id: u32) -> InFile<Span> {}
...
impl TraitApplication {
    pub fn new(trait_params: Vec<TypeId>, impltor: TypeId, impltor_args: Vec<TypeId>) -> Self {
