...
//     let fn_signatures: Vec<FnDeclFirstPass> = root
//         .fn_decls()
//         .map(|fn_decl| ctx.lower_fn_signature(fn_decl))
//         .collect();
//     for (name, _, params, return_ty, _, _) in &fn_signatures {
...
//         .map(
//             |(fn_decl, (name, generic_param_list, params, return_ty, where_clause, _))| {
//                 ctx.lower_fn_decl(
//                     fn_decl,
//                     name,
//                     generic_param_list,
...
#[derive(Debug, Default)]
pub struct Module {
    pub mods: Vec<Idx<ModDecl>>,
    pub enums: Vec<Idx<EnumDecl>>,
    pub structs: Vec<Idx<StructDecl>>,
    pub functions: Vec<Idx<FnDeclFirstPass>>,
}
...
pub fn lower_items(
    ...
    let enums = root
        .enum_decls()
        .map(|enum_decl| item_tree.enums.alloc(ctx.lower_enum_decl(enum_decl)))
        .collect();
    let structs = root
        .struct_decls()
        .map(|struct_decl| item_tree.structs.alloc(ctx.lower_struct_decl(struct_decl)))
        .collect();
    let functions = root
        .fn_decls()
        .map(|fn_decl| item_tree.functions.alloc(ctx.lower_fn_signature(fn_decl)))
        .collect();
...
pub fn lower_item_bodies(
    ...
    let mut ctx = LoweringCtx::new(file_id, interner);
    for f_idx in &module.functions {
        let f = &item_tree.functions[*f_idx];
        ctx.lower_fn_decl(f);
    }
}
