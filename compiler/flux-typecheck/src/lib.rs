...
// use flux_hir::{Block, Call, Expr, FnDecl, HirModule, If, Stmt, Struct, Type, VarDecl};
...
// #[derive(Debug)]
// struct TypeEnv<'a> {
// 	expr_arena: &'a mut Arena<Spanned<Expr>>,
// 	id_counter: usize,
...
// 		self.infer_expr(f.block)?;
// 		Ok(())
// 	}

// 	fn infer_block(&mut self, block: &Block, block_span: Span) -> Result<Spanned<Type>, FluxError> {
// 		self.scopes.push(SymbolTable::default());
// 		let mut span = block_span;
...
// 		self.scopes.pop();
// 		Ok(Spanned::new(Type::Void, span))
// 	}

// 	fn infer_stmt(&mut self, stmt: &Spanned<Stmt>) -> Result<Spanned<Type>, FluxError> {
// 		match &stmt.node {
// 			Stmt::VarDecl(var) => self.infer_var_decl(&Spanned::new(var, stmt.span.clone())),
// 			Stmt::Expr(expr) => self.infer_expr(*expr),
// 			_ => Ok(Spanned::new(Type::Void, stmt.span.clone())),
// 		}
// 	}

// 	fn infer_if(&mut self, if_expr: &If, if_span: Span) -> Result<Spanned<Type>, FluxError> {
// 		self.infer_expr(if_expr.condition)?;
// 		self.infer_expr(if_expr.then)?;
// 		self.infer_expr(if_expr.else_)?;
// 		Ok(Spanned::new(Type::Void, if_span))
// 	}

// 	fn infer_var_decl(&mut self, var: &Spanned<&VarDecl>) -> Result<Spanned<Type>, FluxError> {
...
// 		}
// 		Ok(Spanned::new(Type::Void, var.span.clone()))
// 	}

// 	fn infer_expr(&mut self, idx: Idx<Spanned<Expr>>) -> Result<Spanned<Type>, FluxError> {
// 		let res = match self.expr_arena[idx].node.clone() {
...
// 			Expr::Binary(binary) => {
// 				let lhs_ty = self.infer_expr(binary.lhs)?;
// 				let lhs_id = self.insert(lhs_ty);
// 				let rhs_ty = self.infer_expr(binary.rhs)?;
// 				let rhs_id = self.insert(rhs_ty);
// 				let combined_range = TextRange::new(
// 					self.expr_arena[binary.lhs].span.range.start(),
// 					self.expr_arena[binary.rhs].span.range.end(),
// 				);
// 				let mut result = self.unify(
// 					lhs_id,
// 					rhs_id,
// 					Span::new(combined_range, self.expr_arena[binary.lhs].span.file_id),
// 				);
...
// 				)
// 			}
// 			Expr::Block(block) => self.infer_block(block, self.expr_arena[idx].span.clone())?,
// 			Expr::If(if_expr) => self.infer_if(if_expr, self.expr_arena[idx].span.clone())?,
// 			Expr::Missing => Spanned::new(Type::Unknown, self.expr_arena[idx].span.clone()),
// 		};
// 		Ok(res)
// 	}
...
// 			if let Expr::Path(name) = self.expr_arena[*arg].node.clone() {
